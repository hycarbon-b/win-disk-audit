...
$tempRoot = Join-Path $env:TEMP ('win-disk-audit-npm-test-' + [guid]::NewGuid().ToString('N'))
$target = Join-Path $tempRoot 'skills'
$cli = Join-Path (Split-Path -Parent $PSScriptRoot) 'bin\win-disk-audit.js'

try {
    & node $cli install --target $target
    if ($LASTEXITCODE -ne 0) { throw 'Initial npm CLI installation failed.' }
    $installed = Join-Path $target 'win-disk-audit'
    foreach ($relative in @('SKILL.md', 'agents\openai.yaml', 'scripts\Invoke-WinDiskAudit.ps1', 'scripts\modules\Get-WslUsage.ps1')) {
    ...
    if (-not (Get-ChildItem -LiteralPath $target -Directory | Where-Object Name -like 'win-disk-audit.backup-*')) {
